...
class ContentSyncDestination {
  ...
  private function __construct() { add_action('rest_api_init', [$this, 'registerRoutes']); }
  ...
  public function syncContent(WP_REST_Request $request) {
    ...
    if (!$this->validateJsonData($data)) { return new WP_Error('invalid_data', 'Invalid data structure.', ['status' => 400]); }
    ...
    foreach ($data as $item) {
      ...
      foreach ($item['postMeta'] as $key => $values) {
        foreach ($values as $value) { add_post_meta($postId, sanitize_text_field($key), sanitize_text_field($value)); }
      }
    }
  ...
  private function validateJsonData($data) {
    foreach ($data as $item) {
      if (!isset($item['postTitle']) || !isset($item['postContent'])) { return false; }
    }
